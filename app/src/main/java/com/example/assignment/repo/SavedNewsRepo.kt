...
class SavedNewsRepo  @Inject constructor(
    ...
    suspend fun getSavedNews() = flow<List<Article>> {
        val result = getDatabaseManager().getSavedNews()
        emit(result)
    }

    suspend fun deleteNews(title: String?) = withContext(getDispatcher().io()) {
        getDatabaseManager().deleteNews(title)
    }
}